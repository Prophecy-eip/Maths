...
health_point = input('Enter the health point of the unit: ')
...
with open('tests/initialize_units.rs', 'a') as f:
    ...
    f.write(f'            health_point: {health_point},\n')
    ...
    f.write('        },\n')
    f.write('    );\n')
    f.write(f'    let {name}_modifier_stats: model::Stats = model::Stats::new(\n')
    f.write('        model::GlobalStats {\n')
    f.write('            advance: 0,\n')
    f.write('            march: 0,\n')
    f.write('            discipline: 0,\n')
    f.write('        },\n')
    f.write('        model::DefensiveStats {\n')
    f.write('            health_point: 0,\n')
    f.write('            defense: 0,\n')
    f.write('            resilience: 0,\n')
    f.write('            armour: 0,\n')
    f.write('            aegis: 0,\n')
    f.write('        },\n')
    f.write('        model::OffensiveStats {\n')
    ...
    f.write('            offensive: 0,\n')
    f.write('            strength: 0,\n')
    f.write('            armour_penetration: 0,\n')
    f.write('            agility: 0,\n')
    f.write('        },\n')
    f.write('    );\n')
    f.write(f'    let {name}_modifier: model::Modifier =\n')
    f.write(f'        model::Modifier::new({name}_modifier_stats, false, 0, vec![]);\n')
    f.write(f'    let model_{name}: model::Model =\n')
    f.write(f'        model::Model::new({name}_stats, vec![{name}_modifier]);\n')
    f.write(f'    let {name}: regiment::Regiment = regiment::Regiment::new(model_{name}, {nb_rows}, {nb_columns}, {nb_models});\n')
