...
fn test_warriors_against_heavy_infantry() {
    let attacking_position: fight::AttackPosition = maths::fight::AttackPosition::FRONT;
    ...
    assert_eq!(
        res.get_worst_case().get_defending_regiment().get_points(),
        3
    );
}
...
fn test_heavy_infantry_against_heavy_infantry() {
    ...
    let res: fight::FightPredictionResult =
        fight::compute_turn(attacking_position, &attacking, &defending);
    assert_eq!(res.get_mean_case().get_attacking_regiment().get_points(), 3);
    assert_eq!(res.get_mean_case().get_defending_regiment().get_points(), 2);
    assert_eq!(res.get_best_case().get_attacking_regiment().get_points(), 5);
    assert_eq!(res.get_best_case().get_defending_regiment().get_points(), 3);
    assert_eq!(
        res.get_worst_case().get_attacking_regiment().get_points(),
        4
...
fn test_silexian_spears_against_warriors() {
    ...
    let res: fight::FightPredictionResult =
        fight::compute_turn(attacking_position, &attacking, &defending);
    assert_eq!(res.get_mean_case().get_attacking_regiment().get_points(), 2);
    assert_eq!(res.get_mean_case().get_defending_regiment().get_points(), 6);
    assert_eq!(res.get_best_case().get_attacking_regiment().get_points(), 4);
    assert_eq!(res.get_best_case().get_defending_regiment().get_points(), 5);
    assert_eq!(
        res.get_worst_case().get_attacking_regiment().get_points(),
