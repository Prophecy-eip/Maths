...
pub fn compute_roll_to_hit_shoot(bs: Option<usize>, resilience: usize) -> usize {
    let bs_val: usize = bs.unwrap_or(0);
    let difference: i8 = bs_val as i8 - resilience as i8;
...
fn compute_wound_probability_shooting(
    ...
    let minimum_to_hit: usize = compute_roll_to_hit_shoot(
        attacking_stats.get_ballistic(),
...
pub fn compute_damage_probability_shooting(
    ...
    compute_wound_probability_shooting(attacking_stats, defending_stats)
        * (1.0_f64 - compute_save_probability_shooting(defending_stats, attacking_stats))
}
...
fn compute_save_probability_shooting(
    defending_stats: &stat::Stats,
    attacking_stats: &stat::Stats,
) -> f64 {
...
fn compute_mean_case_shooting(
    ...
    let damage_probability: f64 =
        compute_damage_probability_shooting(attacking_stats, defending_stats);
    let nb_attacks: f64 = (1_f64 * 1.5 * attacking_regiment.get_cols() as f64).round(); // TODO: 1 is default, add the number of attacks depending on the profile of the weapon attached to the unit
...
pub fn compute_case_shooting(
    ...
    let attacking_stats: &stat::Stats = attacking_regiment.get_model().get_boosted_stats();
    let nb_touch: usize = (1_f64 * 1.5 * attacking_regiment.get_cols() as f64).round() as usize; // TODO: 1 is default, add the number of attacks depending on the profile of the weapon attached to the unit
    ...
    if let ComputeCase::MEAN = case {
        return compute_mean_case_shooting(attacking_regiment, defending_regiment);
    }
    match case {
        ComputeCase::BEST => (
            checkpoints.1,
...
mod tests {
    ...
    #[test]
    fn test_hit_shoot() {
        assert_eq!(compute_roll_to_hit_shoot(Some(1), 1), 4);
        assert_eq!(compute_roll_to_hit_shoot(Some(1), 5), 5);
        assert_eq!(compute_roll_to_hit_shoot(Some(1), 9), 6);
        assert_eq!(compute_roll_to_hit_shoot(Some(6), 4), 3);
        assert_eq!(compute_roll_to_hit_shoot(Some(8), 3), 2);
    }
}
